...
function ReadinessCard({ sessions }) {
  ...
  const domainsWithData = Object.values(domainTotals).filter(d => d.total > 0).length;

  return (
    <div className="readiness-card">
      <div className="readiness-card__left">
        <div className="readiness-label">EXAM READINESS</div>
        <div className="readiness-score-num" style={{ color }}>{pct}%</div>
        <div className="readiness-status" style={{ color }}>{label}</div>
        <div className="readiness-bar-track">
          <div
            className="readiness-bar-fill"
            style={{ width: `${pct}%`, background: color }}
          />
          <div className="readiness-bar-target" style={{ left: '70%' }} title="70% passing target" />
        </div>
      ...
      <div className="readiness-card__right">
        {Object.entries(DOMAINS).map(([num, name]) => {
          ...
          const p = d && d.total > 0 ? Math.round((d.correct / d.total) * 100) : null;
          const dot = p === null ? '#555' : p >= 70 ? '#81c784' : p >= 55 ? '#ffb74d' : '#e57373';
          return (
            <div key={num} className="readiness-domain-row">
              <span className="readiness-domain-dot" style={{ background: dot }} />
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="score-chart">
      ...
      {recent.map((s, i) => (
        <circle key={i} cx={x(i)} cy={y(s.pct)} r={4}
          ...
          <title>{new Date(s.date).toLocaleDateString()} · {s.mode} · {s.pct}%</title>
        </circle>
      ))}
  ...
  return (
    <div className="domain-bars">
      {Object.entries(DOMAINS).map(([num, name]) => {
        ...
        const color = pct === null ? 'var(--border)' : pct >= 75 ? '#81c784' : pct >= 60 ? '#ffd54f' : pct >= 45 ? '#ffb74d' : '#e57373';
        return (
          <div key={num} className="domain-bar-row">
            ...
            <div className="domain-bar-track">
              <div className="domain-bar-fill" style={{ width: pct !== null ? `${pct}%` : '0%', background: color }} />
...
function DiffBreakdown({ sessions }) {
  ...
  if (!hasDiff) return <p className="chart-empty">Complete a session to see difficulty breakdown.</p>;
          ...
          <div key={diff} className="diff-row">
            ...
            <div className="diff-row__track">
              <div
                ...
                style={{ width: pct !== null ? `${pct}%` : '0%', background: COLORS[diff] }}
              />
            </div>
...
function Recommendations({ sessions }) {
  ...
  function message(pct, total) {
    ...
    if (total < 10) return `Only ${total} questions attempted — increase coverage.`;
    ...
    if (pct < 75) return 'Close to target — a few more sessions will get you there.';
      ...
      {ranked.map(({ num, name, pct, total }, i) => (
        <div key={num} className="rec-card">
          ...
          <div className="rec-card__body">
            ...
            <div className="rec-card__pct" style={{ color: pct === null ? 'var(--text-muted)' : colors[i] }}>
...
export default function Dashboard({ onHome }) {
  ...
  const passRate = examSessions.length ? Math.round((passCount / examSessions.length) * 100) : null;
    ...
    <div className="dashboard">
      ...
      {sessions.length === 0 ? (
        <div className="dashboard__empty">
          <BarChart2 size={48} strokeWidth={1.25} style={{ color: 'var(--text-muted)' }} />
          <h3>No sessions yet</h3>
          <p>Complete an exam or practice session to see your analytics here.</p>
        ...
        <>
          ...
          <div className="dash-stats">
            ...
            <StatCard value={totalQs.toLocaleString()} label="Questions Answered" />
            <StatCard
            ...
            {passRate !== null && (
              <StatCard
                ...
                valueClass={passRate >= 70 ? 'dash-stat__val--pass' : 'dash-stat__val--fail'}
            ...
            {avgExam !== null && (
              <StatCard
                ...
                valueClass={avgExam >= 700 ? 'dash-stat__val--pass' : 'dash-stat__val--fail'}
              />
            )}
            {recentExam && (
              <StatCard
                ...
                valueClass={recentExam.score >= 700 ? 'dash-stat__val--pass' : 'dash-stat__val--fail'}
              />
            )}
          </div>
          ...
          <div className="dash-section">
            ...
            <p className="rec-sub">Your lowest-performing domains — focus here next.</p>
