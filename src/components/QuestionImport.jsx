...
export default function QuestionImport({ onHome }) {
  ...
  function processText(text, type) {
    setErrors([]);
    setSuccess('');
    const { questions: parsed, errors: errs } =
      type === 'json' ? parseJSON(text) : parseCSV(text);
    ...
    setCustomQs(merged);
    setSuccess(
      `${parsed.length} question${parsed.length > 1 ? 's' : ''} imported successfully!${errs.length > 0 ? ` (${errs.length} skipped due to errors)` : ''}`,
    );
    ...
    <div className="import-screen">
      ...
      {customQs.length > 0 && (
        <div className="import-current">
          <span>
            {customQs.length} custom question{customQs.length > 1 ? 's' : ''}{' '}
            loaded
          </span>
          <button className="btn-link text-fail" onClick={handleClear}>
            Remove all
          </button>
        </div>
      )}
      ...
      {tab === 'format' && (
        <div className="import-body">
          ...
          <div className="import-notes">
            <p>
              <strong>domain:</strong> 1–8 (CISSP domain number)
            </p>
            <p>
              <strong>difficulty:</strong> 1 = Foundation, 2 = Intermediate, 3 =
              Advanced
            </p>
            <p>
              <strong>answer (JSON):</strong> 0-based index (0=A, 1=B, 2=C, 3=D)
            </p>
            <p>
              <strong>answer (CSV):</strong> A, B, C, D or 0, 1, 2, 3
            </p>
            <p>
              <strong>explanation:</strong> optional but recommended
            </p>
          </div>
        </div>
      )}
