    ...
    <div className="review-screen">
      <div className="review-screen__header">
        ...
        <div className="review-screen__summary">
          ...
          <span className="review-stat review-stat--answered">
            {answeredCount} answered
          </span>
          {unanswered > 0 && (
            <span className="review-stat review-stat--warn">
              {unanswered} unanswered
            </span>
          )}
          <span className="review-stat review-stat--flagged">
            {flaggedCount} flagged
          </span>
        </div>
        {flaggedCount > 0 && (
          <p className="review-screen__hint">
            Click any flagged question to review and optionally change your
            answer.
          </p>
        )}
      </div>
        ...
        {questionHistory.map((entry, i) => {
          ...
          return (
            ...
            >
              ...
              {entry.flagged && (
                <span className="review-cell__flag">
                  <Flag size={11} strokeWidth={2} />
                </span>
              )}
              {!hasAnswer && (
                <span className="review-cell__dot review-cell__dot--empty" />
              )}
              {hasAnswer && !entry.flagged && (
                <span className="review-cell__dot review-cell__dot--filled" />
              )}
            </div>
          );
        })}
          ...
          {questionHistory.map((entry, i) =>
            entry.flagged ? (
              ...
              >
                ...
                <span className="flagged-item__domain">
                  {DOMAINS[entry.question.domain]}
                </span>
                <span className="flagged-item__answer">
                  {entry.selectedAnswer !== null
                    ? `Answered: ${OPTION_LABELS[entry.selectedAnswer]}`
                    : 'Unanswered'}
                </span>
                <button className="btn btn--secondary flagged-item__btn">
                  Review →
                </button>
              </div>
            ) : null,
